...
app.post('/auth/login', async (req, res) => {
  ...
  try {
    ...
    // Create JWT token
    const token = jwt.sign({ name: user.name, email: user.email }, process.env.JWT_SECRET, { expiresIn: '60s' });
...
app.get("/all-news", async (req, res) => {
  ...
  let url = `https://newsapi.org/v2/everything?q=${encodeURIComponent(q)}&from=${date}&to=${date}&sortBy=popularity&page=${page}&pageSize=${pageSize}&apiKey=${process.env.API_KEY}`;
  const result = await makeApiRequest(url);
  res.status(result.status).json(result);
});

//featching categories of top headline
...
app.get("/search", async (req,res) => {
  let pageSize = parseInt(req.query.pageSize) || 80;
  let page = parseInt(req.query.page) || 1;
  let q= req.query.searchTerm || "bitcoin";
